...
const BullenSection = () => {
  ...
  const fetchEntries = async () => {
    setLoading(true);
    try {
      const response = await api.get(`/wholesale-bullen?transactionType=${transactionType}&saleType=${activeSaleType}`);
  ...
  const handleGutSubmit = async (e) => {
    ...
    try {
      await api.post('/wholesale-bullen', {
        ...gutFormData,
      ...
      alert(`Gut ${activeType === 'type1' ? 'Vikari' : 'Kharedi'} entry saved successfully!`);
      setGutFormData({
        date: new Date().toISOString().split('T')[0],
        bullenName: '',
        customBullenName: '',
  ...
  const handleKachSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Kach ${activeType === 'type1' ? 'Vikari' : 'Kharedi'} entry saved successfully!`);
      setKachFormData({
        date: new Date().toISOString().split('T')[0],
        formNo: '',
        bullenName: '',
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/wholesale-bullen/${id}`);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Wholesale Bullen');
    XLSX.writeFile(workbook, `Wholesale_Bullen_${transactionType}_${activeSaleType}_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <h2 className="text-2xl font-bold mb-6">Bullen Calculator</h2>
       ...
       {/* Type Selector with Bullen List Button */}
        ...
        <button
          onClick={() => {
            window.location.href = '/wholesale/bullen-list';
          }}
      ...
      {(activeType === 'type1' || activeType === 'type2') && (
        <>
          ...
          <div className="flex gap-4 mb-6">
            ...
            >
              Gut {activeType === 'type1' ? 'Vikhari' : 'Kharedi'}
            </button>
            <button
            ...
            <div className="mb-8">
              ...
              <form onSubmit={handleGutSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="col-span-full">
                  ...
                  >
                    Save Gut {activeType === 'type1' ? 'Vikari' : 'Kharedi'}
                  </button>
                </div>
              </form>
            </div>
          ...
          <div className="mt-8">
            <h3 className="text-lg font-semibold mb-4">
              {activeSaleType === 'gut' ? 'Gut' : 'Kach'} {activeType === 'type1' ? 'Sale' : 'Purchase'} History
            </h3>
            {loading ? (
