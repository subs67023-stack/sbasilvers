...
const JamaKharchCash = () => {
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      const response = await api.get(`/regular-jama-kharch/cash?type=${activeTab}`);
  ...
  const handleDownloadPDF = async () => {
    try {
      const response = await api.get('/regular-jama-kharch/cash/pdf', {
        responseType: 'blob'
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'regular-cash-report.pdf');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/regular-jama-kharch/cash', {
        ...formData,
        type: activeTab
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/regular-jama-kharch/cash/${id}`);
