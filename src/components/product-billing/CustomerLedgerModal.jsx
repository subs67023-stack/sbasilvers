...
export default function CustomerLedgerModal({ customer, onClose }) {
    ...
    const fetchLedger = async () => {
        ...
        try {
            ...
            const response = await api.get(`/product-billing/customers/${customer.id}/ledger?${params}`);
    ...
    return (
        <AnimatePresence>
            ...
            >
                ...
                >
                    {/* Header */}
                    <div className="sticky top-0 bg-gradient-to-r from-teal-600 to-indigo-600 text-white p-6 rounded-t-xl">
                        <div className="flex justify-between items-center">
                            <div>
                                <h2 className="text-2xl font-bold">Product Customer Ledger</h2>
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody className="divide-y divide-gray-200">
                                            {transactions.map((txn, index) => {
                                                ...
                                                // In Product Billing (Sale=Credit, Payment=Debit) logic is inverted from Standard (Sale=Debit, Payment=Credit) usually?
                                                // Wait, Sale (Credit/JAMA) -> Silver Weight POSITIVE. Payment (Debit/KHARCHA) -> Silver Weight NEGATIVE.
                                                // So:
                                                // Credit (Increase Bal) = Positive
                                                // Debit (Decrease Bal) = Negative

                                                const sCredit = sWeight > 0 ? sWeight : 0; // POSITIVE ADDS TO BALANCE (JAMA)
                                                const sDebit = sWeight < 0 ? Math.abs(sWeight) : 0; // NEGATIVE REDUCES BALANCE (KHARCHA)

                                                // Wait, user said Sale = Credit/JAMA/Increase. Correct.
                                                // Payment = Debit/Kharcha/Decrease. Correct.

                                                // BUT in standard ledger UI:
                                                // Debit usually means "You Gave" / "Bill". Credit usually means "You Received" / "Payment".
                                                // Here "Credit" (JAMA) is Sale. "Debit" (KHARCHA) is Payment.
                                                // Let's align headers to Jama/Kharch for clarity.

                                                // Amount Logic
                                                const amt = parseFloat(txn.amount || 0);
                                                const aCredit = amt > 0 ? amt : 0; // Sale
                                                const aDebit = amt < 0 ? Math.abs(amt) : 0; // Payment
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 py-3">
                                                            <span className={`px-2 py-1 rounded text-xs font-medium uppercase ${txn.type === 'sale' ? 'bg-green-100 text-green-800' :
